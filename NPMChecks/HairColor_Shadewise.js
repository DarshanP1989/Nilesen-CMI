// NPM Check: HairColor_Shadewise
const path = require('path');
const { loadSheet, extractMarketProduct } = require(path.join(__dirname, '_shared_npm'));

const CATEGORY    = 'HairColor_Shadewise';
const SHEET_IDX   = 1;
const HEADER_COLS = ['Markets', 'Facts', 'HAIR DYES (1/76)', 'SEG_GCPL', 'MANUFACTURER', 'BRAND', 'SUBBRAND', 'COLOR_GCPL_2'];
const MARKET_COL  = 0;
const PRODUCT_COL = null;  // null = markets only

module.exports = function checkNPM_HairColor_Shadewise(filePaths, masterLookup) {
  const errors = [];
