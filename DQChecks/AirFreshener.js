// DQ: AirFreshener — converted from AirFreshener.py
// Checks: file count=9, sheet name starts '1-', headers on row 10 (Facts,Markets,Products), 36+ months
const { loadWorkbook, getSheet, countMonths, rowStrings, isEmptySheet } = require('./_shared');

module.exports = function validateAirFreshener(filePath, allFiles) {
  const errors = [];
  try {
    const xlsx = allFiles.filter(f => f.toLowerCase().endsWith('.xlsx'));
    if (xlsx.length !== 9) errors.push(`File count: expected 9, found ${xlsx.length}`);
